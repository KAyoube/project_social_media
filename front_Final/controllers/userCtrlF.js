...
exports.isLog = async (req, res, next) => {
  ...
  const userInfo = await fetch(
    `http://${process.env.API_HOST}/api/users/getMe/`,
    {
      headers: {
        Authorization: localStorage.getItem("token"), // Token à récupérer
      },
...
exports.showHome = async (req, res) => {
  if (myProfil.success) {
    ...
    req.flash(
      "success",
      "pour acceder a toutes les infos de cette page veuillez vous connecter ou vous inscrire"
    );
    res.render("home", {
      myProfil,
      message: req.flash("success"),
      message2: req.flash("successLogout"),
    });
  }
};
...
exports.login = async (req, res) => {
  let log = await fetch(`http://${process.env.API_HOST}/api/users/login/`, {
    // Adding method type
    method: "POST",
    ...
    .then((json) => {
      if (json.success) {
        ...
        res.redirect("/feed");
      } else if (json.successAdmin) {
        localStorage.setItem("token", json.token);
        res.redirect(`http://${process.env.API_HOST}/admin/resources/Users`);
      } else {
...
exports.getUserByID = async (req, res) => {
  ...
  if (myPost.success.id === myProfil.success.id) {
    res.redirect("/profil");
  } else {
